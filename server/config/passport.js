...
var Staff = require('../../server/models/staff.js');
...
module.exports = function(passport) {
    ...
    passport.deserializeUser(function(id, done) {
      ...
      passport.use('local_staff-Register', new LocalStrategy({
        ...
        Staff.findOne({ 'email' :  email }, function(err, user) {
            ...
            if (user) {
                ...
                newStaff.password = newStaff.generateHash(req.body.password);
	...
	passport.use('local_staff-login', new LocalStrategy({
        // by default, local strategy uses username and password, we will override with email
        usernameField : 'email',
        passwordField : 'password',
        passReqToCallback : true // allows us to pass back the entire request to the callback
    ...
    function(req, user_id, password, done) { // callback with email and password from our form
        ...
        // find a user whose email is the same as the forms email
        // we are checking to see if the user trying to login already exists
        Staff.findOne({_id: user_id}, function(err, user) {
            // if there are any errors, return the error before anything else
            if (err)
                return done(err);
            if (!user.validPassword(password))
